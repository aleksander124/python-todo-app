...
podman run -d --name api --network=backend -p 8000:8000 -e DB_USER=admin -e DB_PASSWORD=admin -e DB_NAME=todo-app -e DB_HOST=database-prod -e DB_PORT=5432 quay.io/aleksander124/todo-application-api:latest
podman run -d --name frontend --network=frontend -p 3000:3000 quay.io/aleksander124/todo-application-front:latest
```